...
/*****************************************************************************/
/* EffUsesChunks : Uses chunks or misuses chunks                             */
/*****************************************************************************/

bool CEffect::EffUsesChunks()
{
if (!pEffect)
  return false;

return (pEffect->flags & effFlagsProgramChunks) != 0;
}
...
void CVSTHost::SetSampleRate(float fSampleRate)
{
    if (fSampleRate != this->fSampleRate)   /* if no change                      */
    {
        ...
        int i = 0;                          /* inform all loaded plugins         */
        for (; i < GetSize(); ++i)
        {
...
int CVSTHost::LoadPlugin(const char * sName, int nUniqueId)
{
    ...
    int nIndex = -1;                        /* look for empty slot in array      */
...
void CVSTHost::RemoveAt(int nIndex)
{
    ...
    if ( (nIndex < GetSize()) && ((pEffect = GetAt(nIndex)) != NULL) )
    {
        aEffects[nIndex] = 0;
        delete pEffect;
    }
...
bool CVSTHost::OnUpdateDisplay(int nEffect)
{
    ...
    if (pEffect != NULL)
    {
        retVal = pEffect->OnUpdateDisplay();
    }
